...
class CNN(object):
    ...
    def add_input_layer(self, shape=(2,),name="" ):
        ...
        #self.inputLayer = keras.Input(shape=shape,name=name)
    ...
    def append_dense_layer(self, num_nodes,activation="relu",name="",trainable=True):
        """
         ...
         """
        name = self.toLowerCase(name)
        activation = self.toLowerCase(activation)
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        name = self.toLowerCase(name)
        activation = self.toLowerCase(activation)
        padding = self.toLowerCase(padding)

        self.model.add(keras.layers.Conv2D(num_of_filters,kernel_size,activation=activation,
                                            padding=padding,strides=(strides,strides),name=name,trainable=trainable))
    ...
    def set_training_flag(self,layer_numbers=[],layer_names="",trainable_flag=True):
        ...
        if layer_numbers:
          if type(layer_numbers) is list:
            for layerNumber in layer_numbers:
              self.model.layers[layerNumber].trainable = trainable_flag
          else:
            self.model.layers[layer_numbers].trainable = trainable_flag
        else:
          if type(layer_names) is list:
            for layerName in layer_names:
              layerName = self.toLowerCase(layerName)
              self.model.get_layer(name=layerName).trainable = trainable_flag
          else:
            layerName = self.toLowerCase(layer_names)
            self.model.get_layer(name=layerName).trainable = trainable_flag
    ...
    def get_weights_without_biases(self,layer_number=None,layer_name=""):
        ...
        This means that the first layer with activation function is layer zero
         ...
         """
        layer_name = self.toLowerCase(layer_name)
        if layer_number != None and layer_number != 0:
          if layer_number > 0:
            layer_number = layer_number-1
          W = self.model.layers[layer_number].get_weights()
        else:
          try:
            W =  self.model.get_layer(name=layer_name).get_weights()
          except ValueError:
            return None
        if W:
          return W[0]
    ...
    def remove_last_layer(self):
        ...
        """
        lastLayer = self.model.get_layer(index=-1)
        model = self.model
    ...
    def train(self, X_train, y_train, batch_size, num_epochs):
        ...
        self.model.compile(optimizer=self.optimizer,
              loss=self.loss,
              metrics=[self.metric])
        self.history = self.model.fit(x=X_train, y=y_train, batch_size=batch_size,epochs=num_epochs,verbose=0)
